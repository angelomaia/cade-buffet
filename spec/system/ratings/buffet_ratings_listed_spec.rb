...
describe 'All buffet Ratings are listed' do
  it 'in buffet ratings page' do
    ...
    expect(current_path).not_to eq buffet_path(buffet)
    expect(current_path).to eq ratings_buffet_path(buffet)
    expect(page).not_to have_content 'Buffet: Alegria'
    expect(page).to have_content 'Avaliações do Buffet Alegria'
    expect(page).to have_content "Festa de Casamento em #{Order.find(1).date}"
