...
describe 'Buffet access pending order' do
  it 'and sees the chat box' do
    ...
    expect(page).to have_content "Converse com #{user.name}"
  ...
  it 'and send a message to the Buffet' do
    ...
    click_on 'Pedidos'
    click_on 'moIoiXkBAFMXr4RGhn0J'
    fill_in 'Mensagem', with: 'Olá! Como vai?'
    click_on 'Enviar'
    time_sent = BuffetMessage.last.created_at.strftime("%d %b %Y %H:%M")

    expect(page).to have_content "Converse com #{user.name}"
    expect(page).to have_field 'Mensagem'
    expect(page).to have_button 'Enviar'
    expect(page).to have_content 'Olá! Como vai?'
